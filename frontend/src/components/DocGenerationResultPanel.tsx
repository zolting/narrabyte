...
export const mergeConversationEntries = (
	conversation: models.DocConversationMessage[],
	pendingMsg: models.DocConversationMessage | null
): models.DocConversationMessage[] => {
	if (!pendingMsg) {
		return conversation;
	}
	const lastMsg = conversation.at(-1);
	if (
		lastMsg?.role === pendingMsg.role &&
		lastMsg?.content === pendingMsg.content
	) {
		return conversation;
	}
	return [...conversation, pendingMsg];
};
...
export function DocGenerationResultPanel({
	...
	const validChatEntries = useMemo(() => {
		const filteredEntries = conversationEntries.filter(
			(message) => message.role !== "context"
		);
		return mergeConversationEntries(filteredEntries, pendingUserMessage);
	}, [conversationEntries, pendingUserMessage]);
	...
	return (
		<section className="flex h-full flex-col gap-4 overflow-hidden rounded-lg border border-border bg-card p-4">
			...
			{/* adjust grid columns depending on showChat */}
			<div
				className={cn(
					"flex min-h-0 flex-1 flex-col gap-4 overflow-hidden",
					showChat
						? "lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.35fr)]"
			...
			>
				...
				{/* conversation column: only render when showChat is true */}
				{showChat && (
					<div className="flex min-h-0 flex-col gap-3 overflow-hidden rounded-md border border-border bg-muted/40 p-4">
						<div className="flex items-center justify-between gap-2">
							...
							{status === "running" && (
								<span className="text-muted-foreground text-xs">
									{t("common.awaitingResponse", "Awaiting assistant response…")}
								</span>
							)}
						</div>
						<div
						...
						>
							{validChatEntries.length === 0 ? (
								...
								validChatEntries.map((message, index) => {
									const roleClass =
									...
									return (
										<div
											className={cn(
												"rounded-md border border-border p-3 text-foreground text-sm leading-relaxed shadow-xs",
												roleClass
											)}
											key={`${message.role}-${index}`}
										>
											<div className="font-semibold text-foreground/80 text-xs uppercase tracking-wide">
						...
						<form className="space-y-3" onSubmit={handleSubmit}>
							<div className="space-y-1">
								...
								<textarea
									className="min-h-[5.5rem] w-full resize-none rounded-md border border-border bg-background p-3 text-foreground text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-primary disabled:cursor-not-allowed disabled:opacity-60"
							...
							<div className="flex justify-end">
								<Button
									disabled={disableInput || feedback.trim().length === 0}
									type="submit"
								>
									{t("common.send", "Send")}
								</Button>
