...
func (s *generationSessionService) Upsert(projectID uint, sourceBranch, targetBranch, modelKey, provider, messagesJSON string) (*models.GenerationSession, error) {
	sourceBranch = strings.TrimSpace(sourceBranch)
	targetBranch = strings.TrimSpace(targetBranch)
	...
	return s.repo.Upsert(projectID, sourceBranch, targetBranch, modelKey, provider, messagesJSON)
}
