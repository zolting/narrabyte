...
func Grep(ctx context.Context, in *GrepInput) (*GrepOutput, error) {
	runtime.EventsEmit(ctx, events.LLMEventTool, events.NewInfo("Grep: starting"))

	if in == nil {
		runtime.EventsEmit(ctx, events.LLMEventTool, events.NewError("Grep: input is required"))
		return &GrepOutput{
			Title:  "",
	...
	if pattern == "" {
		runtime.EventsEmit(ctx, events.LLMEventTool, events.NewError("Grep: pattern is required"))
		return &GrepOutput{
			Title:  "",
	...
	runtime.EventsEmit(ctx, events.LLMEventTool, events.NewDebug(fmt.Sprintf("Grep: pattern '%s', include '%s'", pattern, strings.TrimSpace(in.Include))))

	base, err := getListDirectoryBaseRoot()
	if err != nil {
		runtime.EventsEmit(ctx, events.LLMEventTool, events.NewError("Grep: project root not set"))
		return &GrepOutput{
			Title:  pattern,
	...
	if search == "" {
		...
		if err != nil {
			runtime.EventsEmit(ctx, events.LLMEventTool, events.NewError(fmt.Sprintf("Grep: invalid project root: %v", err)))
			return &GrepOutput{
				Title:  pattern,
		...
		if err != nil {
			runtime.EventsEmit(ctx, events.LLMEventTool, events.NewError(fmt.Sprintf("Grep: invalid search path: %v", err)))
			return &GrepOutput{
				Title:  pattern,
		...
		if strings.HasPrefix(relToBase, "..") {
			runtime.EventsEmit(ctx, events.LLMEventTool, events.NewWarn("Grep: path escapes the configured project root"))
			return &GrepOutput{
				Title:  pattern,
	...
	runtime.EventsEmit(ctx, events.LLMEventTool, events.NewInfo(fmt.Sprintf("Grep: searching in '%s'", filepath.ToSlash(searchPath))))

	// Ensure directory exists
	info, err := os.Stat(searchPath)
	if err != nil {
		runtime.EventsEmit(ctx, events.LLMEventTool, events.NewError("Grep: path does not exist or is not accessible"))
		return &GrepOutput{
			Title:  pattern,
	...
	if !info.IsDir() {
		runtime.EventsEmit(ctx, events.LLMEventTool, events.NewError("Grep: not a directory"))
		return &GrepOutput{
			Title:  pattern,
	...
	if include != "" {
		runtime.EventsEmit(ctx, events.LLMEventTool, events.NewDebug(fmt.Sprintf("Grep: include filter '%s'", include)))
	}
	...
	if ctx != nil {
		select {
		case <-ctx.Done():
			runtime.EventsEmit(ctx, events.LLMEventTool, events.NewWarn("Grep: canceled"))
			return nil, ctx.Err()
		default:
		}
	}
	...
	if err != nil {
		runtime.EventsEmit(ctx, events.LLMEventTool, events.NewError("Grep: invalid regex pattern"))
		return &GrepOutput{
			Title:  pattern,
	...
	err = filepath.WalkDir(searchPath, func(p string, d fs.DirEntry, err error) error {
		if err != nil {
			if d != nil && d.IsDir() {
				runtime.EventsEmit(ctx, events.LLMEventTool, events.NewWarn(fmt.Sprintf("Grep: skipping unreadable dir '%s'", filepath.ToSlash(p))))
				return fs.SkipDir
			}
			runtime.EventsEmit(ctx, events.LLMEventTool, events.NewWarn(fmt.Sprintf("Grep: unreadable entry '%s'", filepath.ToSlash(p))))
			return nil
		}
		...
		if d.IsDir() {
			...
			if matchIgnoredDir(rel, DefaultIgnorePatterns) {
				runtime.EventsEmit(ctx, events.LLMEventTool, events.NewDebug(fmt.Sprintf("Grep: ignoring dir '%s'", rel)))
				return fs.SkipDir
			}
			return nil
		}
		...
		if bin, berr := isBinaryFile(p); berr == nil && bin {
			runtime.EventsEmit(ctx, events.LLMEventTool, events.NewDebug(fmt.Sprintf("Grep: skipping binary '%s'", filepath.ToSlash(p))))
			return nil
		}
	...
	if err != nil && !errors.Is(err, context.Canceled) {
		runtime.EventsEmit(ctx, events.LLMEventTool, events.NewError(fmt.Sprintf("Grep: traversal error: %v", err)))
		return nil, err
	}

	if len(matches) == 0 {
		runtime.EventsEmit(ctx, events.LLMEventTool, events.NewInfo("Grep: no matches"))
		runtime.EventsEmit(ctx, events.LLMEventTool, events.NewInfo(fmt.Sprintf("Grep: done for '%s'", pattern)))
		return &GrepOutput{
			Title:  pattern,
	...
	runtime.EventsEmit(ctx, events.LLMEventTool, events.NewInfo(fmt.Sprintf("Grep: matched %d item(s)%s", len(matches), map[bool]string{true: " (truncated)", false: ""}[truncated])))
	runtime.EventsEmit(ctx, events.LLMEventTool, events.NewInfo(fmt.Sprintf("Grep: done for '%s'", pattern)))
