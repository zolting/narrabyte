...
// Common event names for consistency across tools.
const (
...
func Glob(ctx context.Context, in *GlobInput) (*GlobOutput, error) {
	runtime.EventsEmit(ctx, EventToolStart, events.NewInfo("Glob: starting"))

	if in == nil {
		runtime.EventsEmit(ctx, EventToolError, events.NewError("Glob: input is required"))
		return &GlobOutput{
			Title:  "",
	...
	if pattern == "" {
		runtime.EventsEmit(ctx, EventToolError, events.NewError("Glob: pattern is required"))
		return &GlobOutput{
	...
	if err != nil {
		runtime.EventsEmit(ctx, EventToolError, events.NewError("Glob: project root not set"))
		return &GlobOutput{
			Title:  strings.TrimSpace(in.Path),
	...
	if search == "" {
		...
		if err != nil {
			runtime.EventsEmit(ctx, EventToolError, events.NewError(fmt.Sprintf("Glob: invalid project root: %v", err)))
			return &GlobOutput{
				Title:  search,
		...
		if err != nil {
			runtime.EventsEmit(ctx, EventToolError, events.NewError(fmt.Sprintf("Glob: invalid search path: %v", err)))
			return &GlobOutput{
				Title:  search,
		...
		if strings.HasPrefix(relToBase, "..") {
			runtime.EventsEmit(ctx, EventToolError, events.NewWarn("Glob: path escapes the configured project root"))
			return &GlobOutput{
				Title:  search,
	...
	runtime.EventsEmit(ctx, EventToolProgress, events.NewInfo(fmt.Sprintf("Glob: searching in '%s'", filepath.ToSlash(searchPath))))

	// Ensure directory exists
	info, err := os.Stat(searchPath)
	if err != nil {
		runtime.EventsEmit(ctx, EventToolError, events.NewError("Glob: path does not exist or is not accessible"))
		return &GlobOutput{
	...
	if !info.IsDir() {
		runtime.EventsEmit(ctx, EventToolError, events.NewError("Glob: not a directory"))
		return &GlobOutput{
			Title:  filepath.ToSlash(searchPath),
	...
	runtime.EventsEmit(ctx, EventToolProgress, events.NewDebug(fmt.Sprintf("Glob: using pattern '%s'", filepath.ToSlash(absPattern))))

	// Expand glob
	matches, err := filepathx.Glob(absPattern)
	if err != nil {
		runtime.EventsEmit(ctx, EventToolError, events.NewError("Glob: invalid glob pattern"))
		return &GlobOutput{
	...
	runtime.EventsEmit(ctx, EventToolProgress, events.NewInfo(fmt.Sprintf("Glob: matched %d file(s)%s", len(files), map[bool]string{true: " (truncated)", false: ""}[truncated])))
	runtime.EventsEmit(ctx, EventToolDone, events.NewInfo(fmt.Sprintf("Glob: done for '%s'", filepath.ToSlash(searchPath))))
